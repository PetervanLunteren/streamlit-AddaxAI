...
existing_pythonpath = os.environ.get("PYTHONPATH", "").strip(":").split(":")  # Strip leading/trailing `:`
updated_pythonpath = list(dict.fromkeys(paths + existing_pythonpath))  # Preserve order and remove duplicates
...
if mode == 0: # simple mode
    ...
    analyse_adv_page = st.Page("analyse_advanced.py", title=txts["analyse_txt"][lang], icon=":material/add:")
    repeat_detection_elimination_page = st.Page("repeat_detection_elimination.py", title="Repeat detection elimination", icon=":material/reset_image:")
    verify_page = st.Page("verify.py", title="Human verification", icon=":material/checklist:")
    depth_estimation_page = st.Page("depth_estimation.py", title="Depth estimation", icon=":material/square_foot:")
    explore_page = st.Page("explore.py", title="Explore results", icon=":material/bar_chart_4_bars:")
    postprocess_page = st.Page("postprocess.py", title=txts["postprocess_txt"][lang], icon=":material/stylus_note:")
    camera_management_page = st.Page("camera_management.py", title="Metadata management", icon=":material/photo_camera:")
    settings_page = st.Page("settings.py", title=txts["settings_txt"][lang], icon=":material/settings:")
    pg = st.navigation([analyse_adv_page, repeat_detection_elimination_page, verify_page, depth_estimation_page, explore_page, postprocess_page, camera_management_page, settings_page])
pg.run()
